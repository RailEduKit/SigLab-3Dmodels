 ...
 * Description: Distant signal component for the Interactive Signalling Laboratory.
 * It is used to create a distant signal for a track and a locking pin.
 */
    ...
    difference(){
        ...
        hull(){
            translate([signal_symbol_size/2,top_cyl_ydif+cylinder_diameter/2,0])
                cylinder(d=cylinder_diameter, h=engraving_height);
            translate([bottom_cyl_xdif+cylinder_diameter/2, signal_symbol_size-engraving_thickness-cylinder_diameter/2,0])
                cylinder(d=cylinder_diameter, h=engraving_height);
            translate([signal_symbol_size-bottom_cyl_xdif-cylinder_diameter/2, signal_symbol_size-engraving_thickness-cylinder_diameter/2,0])
                cylinder(d=cylinder_diameter, h=engraving_height);
        }
    } 
}
...
module signal_lever_distant(){
    // difference(){
        signal_lever(bottom_color="yellow");
        //symbol
...
module visualize_colorBlock_in_body(state){
    ...
    if(state== "-y"){
        rotate([0,0,0])
        translate([wall_thickness_x + move_tolerance, -body_depth/2 + wall_thickness_y+3*move_tolerance,-block_height/2-wall_thickness_z+wall_thickness_z])
