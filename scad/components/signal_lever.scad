 ...
 * Description: Signal handle component is used to create a signal.
 */
...
module signal_lever(top_color="green",bottom_color="red"){
    module handle(){
        union(){
            ...
            translate([0,-handle_depth, (block_height-handle_height)/2])
                color("black")
                cube([block_width, handle_depth+block_depth, handle_height]);
        }
    }
    ...
    module signal_aspect(color_aspect){
        difference(){
            union(){
                color(color_aspect)
                    cube([block_width, block_depth, block_height/2]);
            ...
            translate([-1,0,block_height/2-handle_height/2])
                color(color_aspect)
                cube([block_width+2, block_depth+block_width, block_height/2+handle_height]);
            // color border line
            // translate([0,0,(block_height-handle_height)/2-fine_line]) color("black") cube([fine_line, 2*block_depth, fine_line]);
            // translate([0,0,(block_height+handle_height)/2]) color("black") cube([fine_line, 2*block_depth, fine_line]);
        }
    }
    
    difference(){
        union(){
            translate([block_width,0,block_height]) rotate([0,180,0]) signal_aspect(top_color);
...
module lever_space_cubes(){
    translate([wall_thickness_x,0,z_pos_axis-(handle_height)/2-move_tolerance])
        cube([body_width-2*wall_thickness_x,wall_thickness_y,body_height]); //z=wall_thickness_z+(arrow_block_height-handle_height)/2
