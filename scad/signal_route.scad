...
module bidirectional_arrow(){
    ...
    rotate([0,0,180]) translate([rc_arrow_depth*(3/8)+rc_arrowline_length/2,0,0])
        scale([3/4,(rc_arrow_depth/(2*rc_arrow_depth*sin(120))),1])
        cylinder(engraving_height,rc_arrow_depth,rc_arrow_depth,$fn=3);
}
...
module route_signal(){
    ...
    translate([rc_symbol_xpos,body_depth*(1/6),body_height]) color("white") turnout_locking_symbol(); 
    translate([rc_symbol_xpos,body_depth*(3/6),body_height]) color("white") flank_protection_symbol();
    translate([rc_symbol_xpos,body_depth*(5/6),body_height]) color("white") overlap_symbol();
}
